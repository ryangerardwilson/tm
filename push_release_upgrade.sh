...
run_local_tests() {
  ...
  python3 -m py_compile main.py session_tui.py snapshot_state.py tmux_api.py rgw_cli_contract/*.py || rc=$?

  if [[ -f "tests/test_install_contract.py" ]]; then
    python3 tests/test_install_contract.py || rc=$?
  fi
