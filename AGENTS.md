...
- `install.sh` owns the tmux shortcut snippet for the index-session jump plus the repo-managed root bindings `M-h`, `M-v`, and `M-d`; keep that update idempotent and isolated from unrelated user tmux config.
