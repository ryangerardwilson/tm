...
def test_installer_manages_tmux_shortcut_snippet() -> None:
    ...
    assert 'die "--tmux-key ${tmux_index_key} conflicts with reserved tmux root bindings (M-d, M-h, M-v)"' in script
    ...
    assert 'for key in "$tmux_index_key" "$previous_tmux_index_key" "M-d" "M-h" "M-i" "M-v" "C-i" "Tab" "C-DC" "C-Home" "C-End" "C-Insert" "Insert" "F8" "F9" "F12"; do' in script
    assert 'source-file $TMUX_SNIPPET_FILE' in script
    assert "echo 'bind -n M-h split-window -h'" in script
    assert "echo 'bind -n M-v split-window -v'" in script
